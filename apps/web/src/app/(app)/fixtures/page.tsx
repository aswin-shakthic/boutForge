...
import { canDeleteBracket, canEditPairings } from "@boutforge/shared";
import { FixturesGroupedList } from "@/components/FixturesGroupedList";
import { getAppContext } from "@/lib/app-context";
import { IconAction } from "@/components/ui/IconAction";

export default async function FixturesPage() {
  const { supabase, clubId, membership, isPlatformAdmin } = await getAppContext();
  ...
  const canDelete = membership ? canDeleteBracket(membership.role, isPlatformAdmin) : false;
  ...
  return (
    <div className="space-y-6">
      ...
      <FixturesGroupedList brackets={brackets} canEdit={canEdit} canDelete={canDelete} />
    </div>
  );
}
